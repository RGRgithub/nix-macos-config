...
if [ -f /usr/local/bin/determinate-nixd ] || [ -f /nix/var/nix/profiles/default/bin/determinate-nixd ]; then
    ...
    echo "  3. On a fresh machine 'determinate-nixd' will NOT be in the list yet —"
    echo "     you have to add it manually:"
    echo "       a. Click the '+' button at the bottom of the list"
    echo "          (click the lock icon / authenticate first if prompted)."
    echo "       b. In the file picker, press Cmd+Shift+G to open 'Go to folder'."
    echo "       c. Paste this path and press Enter:"
    echo "            /usr/local/bin/determinate-nixd"
    echo "          (/usr/local/bin is hidden in Finder, which is why Cmd+Shift+G is needed.)"
    echo "       d. Select 'determinate-nixd' and click Open — it will appear in the"
